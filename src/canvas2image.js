...
const Canvas2Image = (() => {
   ...
   */
  let saveAsImage = function(canvas, width, height, type, filename = 'image') {
    if ($support.canvas && $support.dataURL) {
      if (typeof canvas == "string") { canvas = document.getElementById(canvas); }
      if (type == undefined) { type = 'png'; }
      type = fixType(type);

      if (/bmp/.test(`image/${type}`)) {
        ...
        let strData = getDataURL(canvas, `image/${type}`, width, height);
        saveFile(strData.replace(`image/${type}`, downloadMime), filename, type);
      }
    }
  };
  ...
  return {
    ...
    saveAsPNG: function(canvas, width, height, filename = 'image') {
      return saveAsImage(canvas, width, height, 'png', filename);
    },
    saveAsJPEG: function(canvas, width, height, filename = 'image') {
      return saveAsImage(canvas, width, height, 'jpeg', filename);
    },
    saveAsGIF: function(canvas, width, height, filename = 'image') { // BAD WORK
      return saveAsImage(canvas, width, height, 'gif', filename);
    },
    saveAsBMP: function(canvas, width, height, filename = 'image') { // BAD WORK
